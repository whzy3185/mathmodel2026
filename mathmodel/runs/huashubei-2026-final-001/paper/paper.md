...
本项目是比赛截止后的 AI 工作流复现实验。OpenAI Codex 用于题源整理、代码草拟、攻击测试、图表和文字初稿；第二轮攻击 QA 发现了 Q1 跨行身份、无条件周期最小像、前沿漏检和统计措辞问题，主 Agent 据此删除错误模拟代码并改用解析证明。所有权威数值由 `build_corrected_results.py` 生成，依赖与哈希登记在 `evidence/`。若改作正式材料，须由人类作者逐式核验并按适用规则披露。
